...
class WorkflowModel:
    @staticmethod
    def create_workflow(workflow_id, workflow_name, comparison_mode='full', wi_path=None, item_path=None):
        conn = get_db_connection()
        conn.execute('''
            INSERT INTO workflows (id, workflow_name, comparison_mode, wi_document_path, item_master_path)
            VALUES (?, ?, ?, ?, ?)
        ''', (workflow_id, workflow_name, comparison_mode, wi_path, item_path))
        conn.commit()
        conn.close()
